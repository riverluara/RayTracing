	...
	while (loop) {
		...
		case CAMERA:
			...
			for (int i = 0; i < 3; i++) {
				...
				else if (!strcmp(attrType, "near_plane:")) filestr >> near1;
			}
			g_cam.set(e_x, e_y, e_z, l_x, l_y, l_z, g_winWidth, g_winHeight, 45.0f, near1, 1000.0f);
			break;
		case LIGHT:
			filestr >> attrType;
			...
			if (g_box_num > 0) {
				...
				for (int i = 0; i < g_box_num; i++) {
					...
					if (!strcmp(attrType, "conner_position:"))
					{
						...
						if (A.x < B.x)
						{
							g_boxes[i].minPos.x = A.x;
							g_boxes[i].maxPos.x = B.x;
						}
						else
						{
							g_boxes[i].minPos.x = B.x;
							g_boxes[i].maxPos.x = A.x;
						}

						if (A.y < B.y)
						{
							g_boxes[i].minPos.y = A.y;
							g_boxes[i].maxPos.y = B.y;
						}
						else
						{
							g_boxes[i].minPos.y = B.y;
							g_boxes[i].maxPos.y = A.y;
						}

						if (A.z < B.z)
						{
							g_boxes[i].minPos.z = A.z;
							g_boxes[i].maxPos.z = B.z;
						}
						else
						{
							g_boxes[i].minPos.z = B.z;
							g_boxes[i].maxPos.z = A.z;
						}
					}
...
void initialization()
{
	...
	scene.initScene(g_boxes, g_spheres, g_cam, g_light);
}
